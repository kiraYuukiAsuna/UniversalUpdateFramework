...
inline bool generateDifferencePackageManifestFile(std::filesystem::path appversionFolder, UpdatePackageBuildInfo info) {
    ...
    for (auto&directoryEntry: std::filesystem::recursive_directory_iterator(info.NewVersionPath)) {
    ...
    for (auto&directoryEntry: std::filesystem::recursive_directory_iterator(info.OldVersionPath)) {
        if (!directoryEntry.is_directory()) {
            auto relativePath = std::filesystem::relative(directoryEntry.path().string(),
    ...
    differencePackageManifestInfo.Md5 = calcFileMd5(differencePackageFile.string());

    nlohmann::json appdifferencepackagemanifestJson = differencePackageManifestInfo;

    appdifferencepackagemanifestFileFileStream << appdifferencepackagemanifestJson.dump(4);
