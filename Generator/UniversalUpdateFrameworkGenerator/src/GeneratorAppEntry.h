...
inline int generateFullPackage(UpdatePackageBuildInfo &packageBuildInfo) {
	if (!std::filesystem::exists(packageBuildInfo.NewVersionPath)) {
		std::cout << "NewVersionPath:" << packageBuildInfo.NewVersionPath
				  << " not exists.\n";
		return -1;
	}
	...
	std::filesystem::path appChannelPlatformFolder =
		appdataFolder / packageBuildInfo.Channel / packageBuildInfo.Platform;
	if (!std::filesystem::exists(appChannelPlatformFolder)) {
		std::filesystem::create_directories(appChannelPlatformFolder);
	}

	std::filesystem::path appversionFolder =
		appChannelPlatformFolder / packageBuildInfo.AppCurrentVersion;
	...
	if (!generateFullPackageAppVersionFile(appversionFolder,
										   packageBuildInfo)) {
		std::cout << "Error on generate appversion.json.\n";
		return -1;
	}

	if (!generateFullPackageAppManifestFile(appversionFolder,
											packageBuildInfo)) {
...
inline int generateDifferencePackage(UpdatePackageBuildInfo &packageBuildInfo) {
	...
	if (!std::filesystem::exists(packageBuildInfo.OldVersionPath)) {
		std::cout << "OldVersionPath:" << packageBuildInfo.OldVersionPath
				  << " not exists.\n";
		return -1;
	}
	...
	if (!generateDifferencePackageManifestFile(appversionFolder,
											   packageBuildInfo)) {
