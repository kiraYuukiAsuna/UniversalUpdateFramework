...
inline bool generateFullPackageAppVersionFile(
	std::filesystem::path appversionFolder, UpdatePackageBuildInfo info) {
	std::filesystem::path appverionFilePath =
		appversionFolder / "appversion.json";
...
inline bool generateFullPackageAppManifestFile(
	std::filesystem::path appversionFolder, UpdatePackageBuildInfo info) {
	std::filesystem::path appmanifestFilePath =
		appversionFolder / "appmanifest.json";
	...
	for (auto& directoryEntry :
		 std::filesystem::recursive_directory_iterator(newPath)) {
		if (!directoryEntry.is_directory()) {
			...
			manifest.FilePath =
				util::subreplace(relativePath.string(), "\\", "/");
...
inline bool generateFullPackageManifestFile(
	std::filesystem::path appversionFolder, UpdatePackageBuildInfo info) {
	std::filesystem::path fullPackageFile = appversionFolder / "appfullpackage";
	std::cout << fullPackageFile.string() << "\n";
	std::string shellCommand =
		std::format(R"({} -c-zlib "" "{}" "{}")", hdiffzExecuable,
					info.NewVersionPath, fullPackageFile.string());
	std::cout << shellCommand << "\n";

	system(shellCommand.c_str());

	std::filesystem::path appfullpackagemanifestFile =
		appversionFolder / "appfullpackagemanifest.json";
	...
	std::filesystem::path fullpackageFolderPath =
		appversionFolder / "fullpackage";

	std::filesystem::copy(info.NewVersionPath, fullpackageFolderPath,
