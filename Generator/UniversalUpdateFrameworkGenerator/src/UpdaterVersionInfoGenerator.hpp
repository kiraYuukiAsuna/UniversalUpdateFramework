...
AppVersionBriefInfo GenereateUpdaterVersionInfo(
	const std::string& version, const std::string& updaterFullPackagePath) {
	AppVersionBriefInfo appVersionBriefInfo;
	appVersionBriefInfo.version = version;

	for (const auto& file :
		 std::filesystem::directory_iterator(updaterFullPackagePath)) {
		if (!file.is_directory()) {
			auto relativePath = std::filesystem::relative(
				file.path(), std::filesystem::path(updaterFullPackagePath));
