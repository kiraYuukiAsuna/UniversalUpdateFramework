...
#include "src/TypeDefination/UpdatePackageBuild.hpp"

// .\UniversalUpdateFrameworkGenerator.exe -m DifferencePackage -o
// D:\WorkSpace\UnrealEngine\1.1.0 -n D:\WorkSpace\UnrealEngine\1.2.0 -a Aurora
// -v 1.2.0 -b 1.1.0
// .\UniversalUpdateFrameworkGenerator.exe -m FullPackage -n
// D:\WorkSpace\UnrealEngine\1.2.0 -a Aurora -v 1.2.0

int main(int argc, char* argv[]) {
	try {
		cxxopts::Options options("UniversalUpdateFramework Generator",
								 "A tool that help you generate appversion, "
								 "appmanifest, apppackage file.");

		options.add_options()(
			"m,mode", "Generation Mode. (-m FullPackage/DifferencePackage)",
			cxxopts::value<std::string>())("o,oldPath", "Old app path.",
			...
			"b,appbeforeversion", "App before version.",
			cxxopts::value<std::string>())("f,forceupdate", "Force to update.",
										   cxxopts::value<std::string>())(
			"h,help", "Print usage.");
		...
		if (result.count("mode")) {
			...
			if (mode == "FullPackage") {
				...
				if (result.count("appversion")) {
					info.AppCurrentVersion =
						result["appversion"].as<std::string>();
				} else {
				...
				if (result.count("forceupdate")) {
					info.ForceUpdate =
						std::stoi(result["forceupdate"].as<std::string>());
				} else {
				...
				if (result.count("appbeforeversion")) {
					info.AppBeforeVersion =
						result["appbeforeversion"].as<std::string>();
				} else {
