...
int main(int argc, char *argv[]) {
	...
	Seele::Log::Init("logs", "Core.log", "App.log", "EditorConsole.log", true,
					 false);
	auto timestamp = std::chrono::time_point_cast<std::chrono::seconds>(
						 std::chrono::system_clock::now())
						 .time_since_epoch();
	SEELE_INFO_TAG("Main", "App Start At: {}",
				   util::timestampToString(timestamp.count()));

	QApplication a(argc, argv);

	const QString fontPath =
		QString(R"(:/fonts/SourceHanSansCN/SourceHanSansCN-Regular.ttf)");
	const int loadedFontID = QFontDatabase::addApplicationFont(fontPath);
	if (const QStringList loadedFontFamilies =
