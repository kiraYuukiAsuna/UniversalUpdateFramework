...
struct AppSpecification {
    ...
    friend void from_json(const nlohmann::json &j, AppSpecification &value) {
        ...
        value.appUpdateConfigFile = j.value("appUpdateConfigFile", defaultObj.appUpdateConfigFile);
        value.appBackgroundPictureUrl = j.value("appBackgroundPictureUrl", defaultObj.appBackgroundPictureUrl);
        value.appExecutablePath = j.value("appExecutablePath", defaultObj.appExecutablePath);
        value.appExecutableName = j.value("appExecutableName", defaultObj.appExecutableName);
    }
};
...
public:
    ...
    AppSpecification &getConfig() {
        return m_Config;
    }

    void setConfig(AppSpecification &config) {
        m_Config = config;
    }
