...
MainWindow::MainWindow(QWidget* parent)
	...
	connect(ui->LaunchBtn, &QPushButton::clicked, this, [&]() {
		...
		auto executableFileWorkingDir =
			...
			QString::fromStdString(m_AppSpecification.appExecutablePath);
		auto executableFileFullPath =
			executableFileWorkingDir + "/" +
			QString::fromStdString(m_AppSpecification.appExecutableName);
	...
	connect(ui->UpdateBtn, &QPushButton::clicked, this, [&]() {
		...
		int result = QMessageBox::information(
			this, "Info",
			"Are you sure to start update? Please close your application and "
			"save your data before update, or you may lose your data and "
			"update may fail to work!",
...
void MainWindow::refresh() {
	...
	if (m_UpdateConfig.localCurrentVersion == "x.x.x" ||
		m_UpdateConfig.localCurrentVersion.empty()) {
	...
	ui->LocalCurrentVersion->setText(
		QString::fromStdString(localCurrentVersion.getVersionString()));

	try {
		if (auto [result, appVersionContent] = async_simple::coro::syncAwait(
				m_ApiRequest->GetCurrentAppVersion());
			result.getStatus()) {
			m_ServerCurrentAppVersion =
				AppVersionInfo(nlohmann::json::parse(appVersionContent));
			auto serverCurrentVersion =
				Version{m_ServerCurrentAppVersion.AppVersion};
			...
			if (localCurrentVersion < serverCurrentVersion) {
				...
				if (auto [result, appManifestContent] =
						async_simple::coro::syncAwait(
							m_ApiRequest->GetCurrentAppManifest());
					result.getStatus()) {
					AppManifestInfo appManifest =
						nlohmann::json::parse(appManifestContent);
					ui->UpdateReadMe->setMarkdown(
						QString::fromStdString(appManifest.UpdateReadMe));
				}
				if (m_ServerCurrentAppVersion.ForceUpdate) {
					executeUpdate(
						UpdateMode::MultiVersionDifferencePackageUpdate);
				}
			} else {
				ui->UpdateStatus->setText("No Update Available!");
				ui->UpdateReadMe->setMarkdown(
					QString::fromStdString("# No Update Available!"));
			}
		} else {
			QMessageBox::critical(
				this, "Error",
				QString::fromLocal8Bit(result.getErrorMessage()));
...
void MainWindow::initialize() {
	...
	m_ApiRequest =
		new ApiRequest(m_UpdateConfig.host, m_UpdateConfig.appName,
					   m_UpdateConfig.channel, m_UpdateConfig.platform);
}
...
void MainWindow::handleUpdateStatusInfo(UpdateStatusInfo updateStatusInfo) {
	...
	QMetaObject::invokeMethod(
		...
		[this, updateStatusInfo]() {
			...
			ui->UpdateLog->append(QString::fromStdString(
				util::timestampToString(timestamp.count())));
			...
			auto statusName =
				std::string{magic_enum::enum_name(updateStatusInfo.status)};
			switch (updateStatusInfo.status) {
				...
				case UpdateStatus::DownloadingFile: {
					...
					ui->DownloadFileProgress->setValue(
						int((float)updateStatusInfo.DownloadCurrentSize /
							updateStatusInfo.DownloadTotalSize * 100));
					ui->DownloadFileSizeProgress->setText(
						QString::fromStdString(
							std::to_string(
								updateStatusInfo.DownloadCurrentSize / 1024 /
								1024) +
							" / " +
							std::to_string(updateStatusInfo.DownloadTotalSize /
										   1024 / 1024) +
							" MB"));
					break;
				}
				...
				case UpdateStatus::Completed: {
					...
					if (future.getStatus()) {
						...
						QMessageBox::critical(
							this, "Info",
							"Update failed!" + QString::fromStdString(
...
void MainWindow::executeUpdate(UpdateMode mode) {
	...
	m_UpdateFuture =
		std::async(std::launch::async, handleUpdateMode, mode, updateConfigIo,
