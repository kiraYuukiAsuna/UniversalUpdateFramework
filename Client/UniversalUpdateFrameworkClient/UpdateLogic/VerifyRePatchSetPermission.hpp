...
public:
	static async_simple::coro::Lazy<ReturnWrapper> execute(
		ApiRequest& api, AppVersionInfo& appVersion,
		AppManifestInfo& appManifest, const std::string& appPath,
		...
		// for (auto&dirEntry:
		// std::filesystem::recursive_directory_iterator(appPath)) {
		//     if (!dirEntry.is_directory()) {
		//         auto iter =
		//         fileRelativrPathSet.find(std::filesystem::relative(dirEntry.path(),
		//         appPath).string()); if (iter == fileRelativrPathSet.end()) {
		...
		for (auto& fileManifest : appManifest.Manifests) {
			...
			updateStatusCallback(UpdateStatusInfo{
				.status = UpdateStatus::VerifyingFile,
				.CurrentFileName = localFilePath,
			});

			if (!std::filesystem::exists(localFilePath)) {
				SEELE_INFO_TAG(__func__, "Not exist! Download: {}",
							   fileManifest.FilePath);

				auto [result, _] = co_await api.DownloadFileFromFullPackage(
					appVersion.AppVersion, fileManifest.Md5, localFilePath,
			...
			if (fileManifest.Md5 != calcFileMd5(localFilePath)) {
				updateStatusCallback(UpdateStatusInfo{
					...
					.CurrentFileName = fileManifest.FilePath,
				});
				SEELE_INFO_TAG(__func__, "Md5 not equal! Download: {}",
							   fileManifest.FilePath);
				SEELE_INFO_TAG(__func__, "{}",
							   "Download Path " + localFilePath +
								   " ,server md5 = " + fileManifest.Md5 +
								   " ,local md5=" + calcFileMd5(localFilePath));

				if (!ProcessUtil::DeleteFileRecursive(localFilePath)) {
					ReturnWrapper ret{
						false, ErrorCode::DeleteFileFailed,
						"DeleteFileFailed! FilePath: " + localFilePath};
					co_return ret;
				}
				auto [result, _] = co_await api.DownloadFileFromFullPackage(
					appVersion.AppVersion, fileManifest.Md5, localFilePath,
