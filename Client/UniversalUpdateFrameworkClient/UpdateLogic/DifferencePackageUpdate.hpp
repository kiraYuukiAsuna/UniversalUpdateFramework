...
public:
	DifferencePackageUpdate(const std::string& host, const std::string& appName,
							std::string channel, std::string platform,
							std::string appPath, std::string downloadPath,
							std::string oldVersion, std::string newVersion)
		: m_Host(host),
		  m_AppName(appName),
	...
	async_simple::coro::Lazy<ReturnWrapper> execute(
		...
		auto [result1, appVersionContent] =
			co_await m_ApiRequest.GetAppVersion(m_NewVersion);
		...
		auto [result2, appManifestContent] =
			co_await m_ApiRequest.GetAppManifest(m_NewVersion);
		...
		auto [result3, appDifferencePackageManifestContent] =
			co_await m_ApiRequest.GetAppDifferencePackageManifest(m_NewVersion);
		if (!result3.getStatus()) {
			co_return result3;
		}
		DifferencePackageManifestInfo appDifferencePackageManifest =
			nlohmann::json::parse(appDifferencePackageManifestContent);

		ProcessUtil::TerminateProcessByFilePath(m_AppPath);

		std::filesystem::path downloadRootPath =
			m_DownloadPath + "/" + m_AppPath + "/" + m_AppName;
		...
		std::filesystem::path appVersionPath =
			downloadRootPath /
			("appversion_" + appVersion.AppVersion + ".json");
		std::filesystem::path appManifestPath =
			downloadRootPath /
			("appmanifest_" + appManifest.AppVersion + ".json");
		std::filesystem::path appDifferencePackageManifestPath =
			downloadRootPath /
			("appdifferencepackagemanifest_" +
			 appDifferencePackageManifest.AppBeforeVersion + "_to_" +
			 appDifferencePackageManifest.AppCurrentVersion + ".json");

		util::saveToFile(appVersionContent, appVersionPath);
		util::saveToFile(appManifestContent, appManifestPath);
		util::saveToFile(appDifferencePackageManifestContent,
						 appDifferencePackageManifestPath);
		...
		std::string shellCommand = std::format(
			R"({} "" "{}" "{}")", hpatchzExecuable,
			differencePackageFile.string(), uncompresssedPath.string());
		...
		for (auto& file : appDifferencePackageManifest.diff_deletedfiles) {
			...
			updateStatusCallback(
				UpdateStatusInfo{.status = UpdateStatus::DeletingFile,
								 .CurrentFileName = localFilePath});

			if (!ProcessUtil::DeleteFileRecursive(localFilePath)) {
				updateStatusCallback(UpdateStatusInfo{
					.status = UpdateStatus::Warning,
					.WarningMessage = std::string(magic_enum::enum_name(
										  ErrorCode::DeleteFileFailed)) +
									  " , File:" + localFilePath});
		...
		for (auto& file : appDifferencePackageManifest.diff_updatefiles) {
			...
			std::filesystem::path tempUpdatePath =
				downloadRootPath / "DifferenceUpdate" / file;

			std::error_code ec;
			std::filesystem::create_directories(tempUpdatePath.parent_path(),
												ec);
			if (ec) {
				updateStatusCallback(UpdateStatusInfo{
					...
					.ErrorMessage =
						std::string(magic_enum::enum_name(
							ErrorCode::CreateAppDirFailed)) +
						" , File:" + tempUpdatePath.parent_path().string()});
				co_return ReturnWrapper{
					false, ErrorCode::CreateAppDirFailed,
					std::string(
						magic_enum::enum_name(ErrorCode::CreateAppDirFailed)) +
						" , File:" + tempUpdatePath.parent_path().string()};
			}

			updateStatusCallback(
				UpdateStatusInfo{.status = UpdateStatus::UpdatingFile,
								 .CurrentFileName = localFilePath.string()});

			std::string patchShellCommand =
				std::format(R"({} "{}" "{}" "{}")", hpatchzExecuable,
							localFilePath.string(), diffFilePath.string(),
							tempUpdatePath.string());
			...
			if (!ProcessUtil::DeleteFileRecursive(localFilePath)) {
				updateStatusCallback(UpdateStatusInfo{
					...
					.ErrorMessage = std::string(magic_enum::enum_name(
										ErrorCode::DeleteFileFailed)) +
									" , File:" + localFilePath.string()});
				co_return ReturnWrapper{
					false, ErrorCode::DeleteFileFailed,
					std::string(
						magic_enum::enum_name(ErrorCode::DeleteFileFailed)) +
						" , File:" + localFilePath.string()};
			}
			...
			if (ec) {
				updateStatusCallback(UpdateStatusInfo{
					...
					.WarningMessage =
						std::string(
							magic_enum::enum_name(ErrorCode::CopyFileFailed)) +
						" , From File:" + tempUpdatePath.string() +
						" , To File:" + localFilePath.string()

				});
				SEELE_INFO_TAG(__func__, "{}",
							   std::string(magic_enum::enum_name(
								   ErrorCode::CopyFileFailed)) +
								   " , From File:" + tempUpdatePath.string() +
								   " , To File:" + localFilePath.string());
				// co_return ReturnWrapper{
		...
		for (auto& file : appDifferencePackageManifest.diff_newfiles) {
			...
			updateStatusCallback(
				UpdateStatusInfo{.status = UpdateStatus::CreatingFile,
								 .CurrentFileName = localFilePath.string()});

			auto diffFilePath = uncompresssedPath / file;

			std::error_code ec;
			std::filesystem::create_directories(localFilePath.parent_path(),
												ec);
			if (ec) {
				updateStatusCallback(UpdateStatusInfo{
					.status = UpdateStatus::Failed,
					.ErrorMessage =
						std::string(magic_enum::enum_name(
							ErrorCode::CreateAppDirFailed)) +
						" , File:" + localFilePath.parent_path().string()});
				co_return ReturnWrapper{
					false, ErrorCode::CreateAppDirFailed,
					std::string(
						magic_enum::enum_name(ErrorCode::CreateAppDirFailed)) +
		...
		auto result = co_await VerifyRePatchSetPermission::execute(
			m_ApiRequest, appVersion, appManifest, m_AppPath,
			updateStatusCallback);
		if (!result.getStatus()) {
			co_return result;
		}
		...
		updateStatusCallback(
			UpdateStatusInfo{.status = UpdateStatus::DeletingFile,
							 .CurrentFileName = m_DownloadPath});
		if (!ProcessUtil::DeleteFileRecursive(m_DownloadPath)) {
			updateStatusCallback(UpdateStatusInfo{
				.status = UpdateStatus::Failed,
				.ErrorMessage = std::string(magic_enum::enum_name(
									ErrorCode::DeleteDownloadDirFailed)) +
								" , File:" + m_DownloadPath});
	...
	ApiRequest& getApi() { return m_ApiRequest; }
